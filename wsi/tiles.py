...
def summary_and_tiles(slide_num, cfg, display=True, save_summary=False, save_data=True, save_top_tiles=True):
  """
...
def score_tiles(slide_num, cfg, np_img=None, dimensions=None, small_tile_in_tile=False):
  """
  ...
  row_tile_size = round(ROW_TILE_SIZE / cfg.SLIDE.SCALE_FACTOR)  # use round?
  col_tile_size = round(COL_TILE_SIZE / cfg.SLIDE.SCALE_FACTOR)  # use round?
  ...
  for t in tile_indices:
    count += 1  # tile_num
    # row start, row end, col start, col end, row idx, col idx
    ...
    if amount == TissueQuantity.HIGH:
      ...
      none += 1
    o_c_s, o_r_s = slide.small_to_large_mapping((c_s, r_s), (o_w, o_h), cfg)
    o_c_e, o_r_e = slide.small_to_large_mapping((c_e, r_e), (o_w, o_h), cfg)
...
def image_range_to_tiles(start_ind, end_ind, cfg, display=False, save_summary=True, save_data=True, save_top_tiles=True):
  """
  ...
  for slide_num in range(start_ind, end_ind + 1):
    tile_summary = summary_and_tiles(slide_num, cfg, display, save_summary, save_data, save_top_tiles)
...
def multiprocess_filtered_images_to_tiles(cfg, display=False, save_summary=True, save_data=True, save_top_tiles=True,
                                          html=True, image_num_list=None):
  """
  ...
  for num_process in range(1, num_processes + 1):
    ...
    if image_num_list is not None:
      ...
      tasks.append((sublist, cfg, display, save_summary, save_data, save_top_tiles))
      print("Task #" + str(num_process) + ": Process slides " + str(sublist))
    else:
      tasks.append((start_index, end_index, cfg, display, save_summary, save_data, save_top_tiles))
  ...
  # with open(cfg.TILE.PATCH_IDX, 'wt', encoding='utf-8') as outfile:
