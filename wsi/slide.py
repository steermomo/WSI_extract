...
from yacs.yacs_config import CfgNode
from configs import cfg as default_config
...
def get_training_image_path(slide_number, cfg: default_config, large_w=None, large_h=None, small_w=None, small_h=None) -> str:
    """
    ...
    #         cfg.SLIDE.SRC_DIR, cfg.SLIDE.PREFIX  + padded_sl_num + "*." + DEST_TRAIN_EXT)
    ...
    if large_w is None and large_h is None and small_w is None and small_h is None:
        # 获取 slide src路径
        img_path = glob.glob1(cfg.SLIDE.SRC_DIR, cfg.SLIDE.PREFIX + "*." + cfg.SLIDE.SRC_EXT)[slide_number]
    else:
        # 获取 slide 缩放后的路径
        img_basename = os.path.basename(glob.glob1(cfg.SLIDE.SRC_DIR, "*" + cfg.SLIDE.SRC_EXT)[slide_number])
        img_basename = img_basename.rstrip(cfg.SLIDE.SRC_EXT)
        img_path = os.path.join(cfg.SLIDE.DEST_DIR, img_basename + "-" + str(
                    cfg.SLIDE.SCALE_FACTOR) + "x-" + cfg.SLIDE.DEST_SUFFIX + str(
                    large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(small_h) + "." + cfg.SLIDE.DEST_EXT)
    return img_path


def get_training_thumbnail_path(slide_number, cfg: default_config, large_w=None, large_h=None, small_w=None, small_h=None):
    """
    ...
    if large_w is None and large_h is None and small_w is None and small_h is None:
        img_path = glob.glob1(cfg.SLIDE.DEST_THUMBNAIL_DIR, "*" + THUMBNAIL_EXT)[slide_number]
    else:
        img_basename = os.path.basename(glob.glob1(cfg.SLIDE.DEST_THUMBNAIL_DIR, "*" + THUMBNAIL_EXT)[slide_number])
        img_basename = img_basename.rstrip(cfg.SLIDE.THUMBNAIL_EXT)
        img_path = os.path.join(cfg.SLIDE.DEST_THUMBNAIL_DIR, f"{img_basename}-{cfg.SLIDE.SCALE_FACTOR}x-{large_w}x{large_h}-{small_w}x{small_h}.{cfg.SLIDE.THUMBNAIL_EXT}")
    return img_path
...
def get_filter_thumbnail_path(slide_number, cfg, filter_number, filter_name_info):
    """
    ...
    dir = cfg.FILTER.THUMBNAIL_DIR
    if not os.path.exists(dir):
        os.makedirs(dir)
    img_path = os.path.join(dir, get_filter_image_filename(
        slide_number, cfg, filter_number, filter_name_info, thumbnail=True))
    return img_path


def get_filter_image_filename(slide_number, cfg, filter_number, filter_name_info, thumbnail=False):
    """
    ...
    if thumbnail:
        # ext = THUMBNAIL_EXT
        ext = cfg.THUMBNAIL_EXT
    else:
        ext = cfg.SLIDE.DEST_EXT
    # padded_sl_num = str(slide_number).zfill(3)
    ...
    img_basename = get_training_image_path(slide_number, cfg)
    img_basename = os.path.basename(img_basename).rstrip(cfg.SLIDE.SRC_EXT)
    img_filename = cfg.SLIDE.PREFIX + img_basename +  "-" + padded_fi_num + \
                   "-" + FILTER_SUFFIX + filter_name_info + "." + ext
    return img_filename
...
def get_filter_image_result(slide_number, cfg: default_config):
    """
    ...
    img_path = os.path.join(cfg.FILTER.DIR, "-" + str(
        cfg.SLIDE.SCALE_FACTOR) + "x-" + cfg.FILTER.PREFIX + str(large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(
        small_h) + "-" + cfg.FILTER.RESULT_TEXT + "." + cfg.SLIDE.DEST_EXT)
    return img_path
...
def small_to_large_mapping(small_pixel, large_dimensions, cfg):
    """
    ...
    """
    # TODO: why use SCALE_FACTOR
    SCALE_FACTOR = cfg.SLIDE.SCALE_FACTOR
...
def training_slide_to_image(slide_number: int, cfg: default_config) -> None:
    """
    ...
    if not os.path.exists(cfg.SLIDE.DEST_DIR):
        os.makedirs(cfg.SLIDE.DEST_DIR)
    img.save(img_path)

    thumbnail_path = get_training_thumbnail_path(
        slide_number, large_w, large_h, new_w, new_h)
    save_thumbnail(img, cfg.SLIDE.THUMBNAIL_SIZE, thumbnail_path)


def slide_to_scaled_pil_image(slide_number, cfg: default_config):
    """
    ...
    new_w = math.floor(large_w / cfg.SLIDE.SCALE_FACTOR)
    new_h = math.floor(large_h / cfg.SLIDE.SCALE_FACTOR)
    level = slide.get_best_level_for_downsample(cfg.SLIDE.SCALE_FACTOR)
...
def get_num_training_slides(cfg: default_config):
    """
    ...
    """
    num_training_slides = len(glob.glob1(cfg.SLIDE.SRC_DIR, "*." + cfg.SLIDE.SRC_EXT))
    # num_training_slides = len(glob.glob(cfg.SLIDE.SRC_DIR, "*." + cfg.SLIDE.SRC_EXT))
    return num_training_slides


def training_slide_range_to_images(start_ind: int, end_ind: int, cfg: default_config) -> tuple[int, int]:
    """
    ...
    for slide_num in range(start_ind, end_ind + 1):
        training_slide_to_image(slide_num, cfg)
...
def multiprocess_training_slides_to_images(cfg: CfgNode):
    """
    ...
    for num_process in range(1, num_processes + 1):
        ...
        tasks.append((start_index, end_index, cfg))
